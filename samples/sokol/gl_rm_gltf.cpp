...
#include "shaders/generated/cgltf-sapp.glsl.h"
...
static const char *gltf_path = "glTF/";
// static const char *gltf_path = "DamagedHelmet/";
static const char *gltf_filename = "DamagedHelmet.gltf";
// static const char *gltf_path = "the_mill/";
// static const char *gltf_filename = "scene.gltf";
...
uint8_t *read_file_to_buffer(const char *filename, int &len) {
    ...
    if (fseek(fp, 0L, SEEK_END) == 0) {
        ...
        size_t new_len = fread(data, sizeof(uint8_t), len, fp);
        if (ferror(fp) != 0) {
...
static void gltf_parse(const char *filename);
...
static vertex_buffer_mapping_t create_vertex_buffer_mapping_for_gltf_primitive(const cgltf_data *gltf,
                                                                               const cgltf_primitive *prim);
static int create_sg_pipeline_for_gltf_primitive(const cgltf_data *gltf, const cgltf_primitive *prim,
                                                 const vertex_buffer_mapping_t *vbuf_map);
...
void gltf_scene_init(void) {
    ...
    state.pass_actions.ok =
        (sg_pass_action){.colors[0] = {.action = SG_ACTION_CLEAR, .val = {0.0f, 0.569f, 0.918f, 1.0f}}};
    state.pass_actions.failed =
    ...
    state.point_light = (light_params_t){.light_pos = HMM_Vec3(10.0, 10.0, 10.0),
                                         .light_range = 200.0,
    ...
    // });
    char tmp[512];
    strcpy(tmp, gltf_path);
    gltf_parse(strcat(tmp, gltf_filename));
    ...
    state.placeholders.white =
        sg_make_image((sg_image_desc){.width = 8,
                                      .height = 8,
        ...
        for (int node_index = 0; node_index < state.scene.num_nodes; node_index++) {
            ...
            for (int i = 0; i < mesh->num_primitives; i++) {
                ...
                if (mat->is_metallic) {
                    ...
                    sg_apply_uniforms(SG_SHADERSTAGE_FS, SLOT_metallic_params, &mat->metallic.fs_params,
...
static void gltf_parse(const char *filename) { // void* ptr, uint64_t num_bytes) {
    cgltf_options options = {.type = cgltf_file_type_invalid};
    cgltf_data *data = 0;
    // const cgltf_result result = cgltf_parse(&options, ptr, num_bytes, &data);
    const cgltf_result result = cgltf_parse_file(&options, filename, &data);
...
static void gltf_parse_buffers(const cgltf_data *gltf) {
    ...
    for (cgltf_size i = 0; i < gltf->buffers_count; i++) {
        ...
        char tmp[512];
        strcpy(tmp, gltf_path);
        uint8_t *buffer_ptr = read_file_to_buffer(strcat(tmp, gltf_buf->uri), fetched_size);
        printf("%d\n", fetched_size);
        create_sg_buffers_for_gltf_buffer(i, (const uint8_t *)buffer_ptr, (int)fetched_size);
        free(buffer_ptr);
    }
}
...
static void gltf_parse_images(const cgltf_data *gltf) {
    ...
    for (cgltf_size i = 0; i < gltf->images_count; i++) {
        ...
        // gltf_image_fetch_userdata_t user_data = {.image_index = i};
        // sfetch_send(&(sfetch_request_t){.path = gltf_img->uri,
        ...
        char tmp[512];
        strcpy(tmp, gltf_path);
        uint8_t *buffer_ptr = read_file_to_buffer(strcat(tmp, gltf_img->uri), fetched_size);
        create_sg_images_for_gltf_image(i, buffer_ptr, fetched_size);
        free(buffer_ptr);
    }
}
...
static void create_sg_buffers_for_gltf_buffer(int gltf_buffer_index, const uint8_t *bytes, int num_bytes) {
    for (int i = 0; i < state.scene.num_buffers; i++) {
        ...
        if (p->gltf_buffer_index == gltf_buffer_index) {
            ...
            sg_init_buffer(state.scene.buffers[i],
                           (sg_buffer_desc){.type = p->type, .size = p->size, .content = bytes + p->offset});
        }
    }
}
...
static void create_sg_images_for_gltf_image(int gltf_image_index, const uint8_t *bytes, int num_bytes) {
    for (int i = 0; i < state.scene.num_images; i++) {
        ...
        if (p->gltf_image_index == gltf_image_index) {
            ...
            stbi_uc *pixels =
                stbi_load_from_memory(bytes, num_bytes, &img_width, &img_height, &num_channels, desired_channels);
            if (pixels) {
                state.scene.images[i] = sg_make_image((sg_image_desc){
                    .width = img_width,
                    .height = img_height,
                    .pixel_format = SG_PIXELFORMAT_RGBA8,
...
static sg_layout_desc create_sg_layout_for_gltf_primitive(const cgltf_data *gltf, const cgltf_primitive *prim,
                                                          const vertex_buffer_mapping_t *vbuf_map) {
...
static bool pipelines_equal(const pipeline_cache_params_t *p0, const pipeline_cache_params_t *p1) {
    ...
    for (int i = 0; i < SG_MAX_VERTEX_ATTRIBUTES; i++) {
        ...
        if ((a0->buffer_index != a1->buffer_index) || (a0->offset != a1->offset) || (a0->format != a1->format)) {
            return false;
        }
    }
    return true;
...
static int create_sg_pipeline_for_gltf_primitive(const cgltf_data *gltf, const cgltf_primitive *prim,
                                                 const vertex_buffer_mapping_t *vbuf_map) {
    pipeline_cache_params_t pip_params = {.layout = create_sg_layout_for_gltf_primitive(gltf, prim, vbuf_map),
            ...
            .blend =
                {
                    ...
                    .src_factor_rgb = pip_params.alpha ? SG_BLENDFACTOR_SRC_ALPHA : SG_BLENDFACTOR_ONE,
                    .dst_factor_rgb = pip_params.alpha ? SG_BLENDFACTOR_ONE_MINUS_SRC_ALPHA : SG_BLENDFACTOR_ZERO,
                    .color_write_mask = uint8_t(pip_params.alpha ? SG_COLORMASK_RGB : SG_COLORMASK_RGBA),
                },
            .rasterizer = {
...
static vs_params_t vs_params_for_node(int node_index) {
    ...
    vs_params_t vs_params = {
        .model = model_transform, .view_proj = state.camera.view_proj, .eye_pos = state.camera.eye_pos};
    return vs_params;
}
